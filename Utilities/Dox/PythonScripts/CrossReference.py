...
class AbstractVariable:
    def __init__(self, name, prefix, lineOffsets):
        self._name = name
        self._prefix = prefix
        self._notKilledExp = prefix and prefix.strip() == NOT_KILLED_EXPLICITLY_VALUE
...
class Routine(object):
    ...
    def getFilteredExternalReference(self, filterList):
        output = dict()
        for routineDict in self._calledRoutines.itervalues():
            for (routine, callTagDict) in routineDict.iteritems():
    ...
    def addCallDepRoutines(self, depRoutine, callTag, lineOccurences, isCalled=True):
        ...
        if package is None:
          # TODO: Routine' has no 'getPackageByName' member
    ...
    def addEntryPoint(self, entryPt, comm, icrJSON):
        ...
        if entryPt not in self._entryPoints:
            if icrJSON:
              icrEntries = generateEntryList(entryPt.split("(")[0], icrJSON)
...
#===============================================================================
class FileManField(JSONEncoder):
...
class Global(FileManFile):
    ...
    def getTotalNumberOfReferencedGlobals(self):
        return self.totalReferencedGlobals
    ...
    def addPointedToByFile(self, globalVar, fieldNo, subFileNo=None):
        self.__addReferenceGlobalFilesCommon__(globalVar, fieldNo, subFileNo, True)
        globalVar.__addPointedToFiles__(self, fieldNo, subFileNo)

    def __addPointedToFiles__(self, globalVar, fieldNo, subFileNo):
        self.__addReferenceGlobalFilesCommon__(globalVar, fieldNo, subFileNo, False)

    def __addReferenceGlobalFilesCommon__(self, globalVar, fieldNo, subFileNo, pointedToBy=True):
        if not globalVar:
            return
        ...
        if globalVar not in depFileDict[package]:
            depFileDict[package][globalVar] = []
            if pointedToBy:
              self._totalReferredGlobals = self._totalReferredGlobals + 1
              globalVar.totalReferencedGlobals = globalVar.totalReferencedGlobals + 1
        depFileDict[package][globalVar].append((fieldNo, subFileNo))
...
class Package(object):
    ...
    def __init__(self, packageName):
        ...
        self.routineDependents = dict()
        self._globalRoutineDependencies = dict()
        self.globalRoutineDependendents = dict()
        self._objectDependencies = dict()
        self.objectDependents = dict()
        self._globalDependencies = dict()
        self._globalGlobalDependencies = dict()
        self.globalGlobalDependendents = dict()
        self.globalDependents = dict()
        self._fileManDependencies = dict()
        self.fileManDependents = dict()
    ...
    #**************************************
    def getPackageComponent(self, componentType, ien):
      return self._objects[componentType][ien]

    def getAllPackageComponents(self, componentType="*"):
      if componentType == "*":
        return self._objects
      return self._objects[componentType]

    def addPackageComponent(self, componentType, value):
      value.addObjectType(componentType)
      self._objects[componentType][value.getIEN()] = value
          ...
          for obj in allObjs[key]:
            ...
            for package in calledRoutines.iterkeys():
                if package and package != self:
                    ...
                    if self not in package.objectDependents:
                        ...
                        package.objectDependents[self] = (set(), set())
                    package.objectDependents[self][0].add(allObjs[key][obj])
                    package.objectDependents[self][1].update(calledRoutines[package])
        ...
        for globalEntry in self._globals.itervalues():
            ...
            for package in calledRoutines.iterkeys():
                if package and package != self:
                    ...
                    if self not in package.globalRoutineDependendents:
                        # the first set consists of all called _calledRoutine in the package
                        # the second set consists of all caller routines in that package
                        package.globalRoutineDependendents[self] = (set(), set())
                    package.globalRoutineDependendents[self][0].add(globalEntry)
                    package.globalRoutineDependendents[self][1].update(calledRoutines[package])
            ...
            for globalVar in referredGlobals.itervalues():
                ...
                if package != self:
                    ...
                    if self not in package.globalGlobalDependendents:
                        package.globalGlobalDependendents[self] = (set(), set())
                    package.globalGlobalDependendents[self][0].add(globalEntry)
                    package.globalGlobalDependendents[self][1].add(globalVar)
        for routine in self._routines.itervalues():
            ...
            for package in calledRoutines.iterkeys():
                if package and package != self:
                    ...
                    if self not in package.routineDependents:
                        # the first set consists of all called _calledRoutine in the package
                        # the second set consists of all caller routines in that package
                        package.routineDependents[self] = (set(), set())
                    package.routineDependents[self][0].add(routine)
                    package.routineDependents[self][1].update(calledRoutines[package])
            ...
            for globalVar in referredGlobals.itervalues():
                ...
                if package != self:
                    ...
                    if self not in package.globalDependents:
                        package.globalDependents[self] = (set(), set())
                    package.globalDependents[self][0].add(routine)
                    package.globalDependents[self][1].add(globalVar)
            # based on fileman db calls
            filemanDbCallGbls = routine.getFilemanDbCallGlobals()
            for filemanDbGbl in filemanDbCallGbls.keys():
                ...
                if package != self:
                    ...
                    if self not in package.fileManDbDependents:
                        package.fileManDbDependents[self] = (set(), set())
                    package.fileManDbDependents[self][0].add(routine)
                    package.fileManDbDependents[self][1].add(filemanDbGbl)
    ...
    def generateFileManFileBasedDependencies(self):
        ...
        for globalVar in self._globals.itervalues():
            if not globalVar.isFileManFile(): # only care about the file man file now
                continue
            pointerToFiles = globalVar.getAllReferredFileManFiles()
            for package in pointerToFiles.iterkeys():
                if package != self:
                    ...
                    self._fileManDependencies[package][0].add(globalVar)
                    self._fileManDependencies[package][1].update(pointerToFiles[package])
                    if self not in package.fileManDependents:
                        package.fileManDependents[self] = (set(), set())
                    package.fileManDependents[self][0].add(globalVar)
                    package.fileManDependents[self][1].update(pointerToFiles[package])
    ...
    def __correctFileManFilePointerDependencies__(self):
        for globalVar in self._globals.itervalues():
            ...
            if not globalVar.isFileManFile():
                continue
            self.__checkIndividualFileManPointers__(globalVar)

    def __checkIndividualFileManPointers__(self, globalVar, subFile=None):
        currentGlobal = globalVar
        if subFile:
        ...
        if allFileManFields:
            for field in allFileManFields.itervalues():
                if field.isFilePointerType():
                    ...
                    self.__checkFileManPointerField__(field, fileManFile, globalVar, subFile)
                    continue
        ...
        if not subFile:
            ...
            allSubFiles = globalVar.getAllSubFiles()
            if not allSubFiles:
                return
            for val in allSubFiles.itervalues():
                self.__checkIndividualFileManPointers__(globalVar, val)

    def getPackageComponentDependencies(self):
        return self._objectDependencies
    def getPackageComponentDependents(self):
        return self.objectDependents
...
def generateEntryList(entries, totalEntries):
  """ Remove the "$$" notation from entry points and
  remove any given paramters. """
...
def checkFileManPointerField(field, fileManFile, globalVar, subFile):
    if not fileManFile:
        logger.warning("Invalid fileMan File pointed to by field:[%s], Global:[%s], subFile:[%s]" %
                          (field, globalVar, subFile))
        return
    # make sure that fileManFile is in the globalVar's filePointers
    fieldNo = field.getFieldNo()
    subFileNo = None
    if subFile:
        subFileNo = subFile.getFileNo()
    if not globalVar.hasPointerToFileManFile(fileManFile, fieldNo, subFileNo):
        logger.warning("Global[%r] does not have a pointer to [%r] at [%s]:[%s]" %
                          (globalVar, fileManFile, fieldNo, subFileNo))
        fileManFile.addPointedToByFile(globalVar, fieldNo, subFileNo)
        return
    if not fileManFile.isPointedToByFileManFile(globalVar, fieldNo, subFileNo):
        logger.warning("FileMan file[%r] does not pointed to by [%r] at [%s]:[%s]" %
                          (fileManFile, globalVar, fieldNo, subFileNo))
        fileManFile.addPointedToByFile(globalVar, fieldNo, subFileNo)
        return
