...
def _getRPCHRefLink(rpcName, icrEntry, **kargs):
    if rpcName in RPC_NAME_TO_IEN_MAPPING:
        ...
        rpcFilename = os.path.join(VIVIAN_URL, RPC_FILE_NO, filename)
...
def _generateICRIndividualPagePDF(icrJson, date, pdfOutDir):
    ...
    # Table
    _icrDataEntryToPDF(pdf, icrJson)
    ...
    for field in fieldList:
        if field in icrJson: # we have this field
            ...
            if isSubFile(field):
                ...
                output.write("<td>\n")
                output.write ("<ol>\n")
                _icrSubFileToHtml(output, value, field)
                output.write ("</ol>\n")
                output.write("</td>\n")
                output.write ("</tr>\n")
                continue
...
def _icrDataEntryToPDF(pdf, icrJson):
    ...
    for field in fieldList:
        if field in icrJson: # we have this field
            ...
            if "GLOBAL REFERENCE" == field:
                _writeGlobalReferenceToPDF(value, pdf)
                continue
            ###############################################################
            if "COMPONENT/ENTRY POINT" == field:
                _writeComponentEntryPointToPDF(value, pdf)
                continue
            ###############################################################
...
def _writeGlobalReferenceToPDF(section, pdf):
...
def _writeComponentEntryPointToPDF(section, pdf):
...
def _icrSubFileToHtml(output, icrJson, subFile):
